describe('logic', function () {
    describe('registerUser', function () {
        it('succeds a new user', function () {
        ...
        it('fails on empty name', function () {
            ...
            expect(errorThrown).toBeInstanceOf(Error)
            expect(errorThrown).toBe('name >< is empty or blank')
        })
        ...
        it('fails on non string birthdate', function () {
            ...
            expect(errorThrown).toBeInstanceOf(TypeError)
            expect(errorThrown).toBe('birthdate 123 is not a string')
        })
        ...
        it('fails on incorrect birthdate format', function () {
            ...
            expect(errorThrown).toBeInstanceOf(Error)
            expect(errorThrown).toBe('birthdate 2000/01/01 does not have a valid format')
        })

        it('fails on non-string email', function () {
            var errorThrown
            ...
            expect(errorThrown).toBeInstanceOf(TypeError)
            expect(errorThrown).toBe('email 123 is not a string')
        })

        it('fails on non formatted email', function () {
            var errorThrown
            ...
            expect(errorThrown).toBeInstanceOf(TypeError)
            expect(errorThrown).toBe('email peperoni.c is not an email')
        })
    ...
    describe('loginUser', function () {
        it('succeeds on existing user and correct credentials', function () {
            ...
            var user = db.user.findOne(function (user) {
                return user.username === 'peperoni'
            })
